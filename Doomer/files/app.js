   ...
   setSpriteSheet(name, direction, mode) {
      ...
      if (mode === "scared") {
         this.animationTarget.style.backgroundImage =
            "url(app/style/graphics/" + `spriteSheets/characters/ghosts/scared_${this.scaredColor}.svg)`;
      } else if (mode === "eyes") {
         this.animationTarget.style.backgroundImage =
   ...
   handleIdleMovement(elapsedMs, position, velocity) {
      ...
      if (this.idleMode === "leaving") {
         if (position.x === 13.5 && position.y > 10.8 && position.y < 11) {
            ...
            this.direction =
               position.x < 13.5 ? this.characterUtil.directions.right : this.characterUtil.directions.left;
         }
      }
...
class Pacman {
   ...
   changeDirection(newDirection, startMoving) {
      ...
      this.pacmanArrow.style.backgroundImage =
         "url(app/style/graphics/" + `spriteSheets/characters/pacman/arrow_${this.desiredDirection}.svg)`;
...
class GameCoordinator {
   constructor() {
      ...
      this.viewportHtml = '<meta name="viewport" content="width=device-width, initial-scale=1.0">';
   ...
   toggleHiddenPage(makeHidden = false) {
      if (makeHidden) {
         this.body.classList.add("_hidden");
         this.headEl.removeChild(this.viewportMeta);
      } else {
         this.body.classList.remove("_hidden");
         this.headEl.insertAdjacentHTML("beforebegin", this.viewportHtml);
         this.viewportMeta = document.querySelector('meta[name="viewport"]');
      }
   }
...
class CharacterUtil {
   ...
   changingGridPosition(oldPosition, position) {
      return (
         Math.floor(oldPosition.x) !== Math.floor(position.x) || Math.floor(oldPosition.y) !== Math.floor(position.y)
      );
   }
   ...
   advanceSpriteSheet(character) {
      const { msSinceLastSprite, animationTarget, backgroundOffsetPixels } = character;
